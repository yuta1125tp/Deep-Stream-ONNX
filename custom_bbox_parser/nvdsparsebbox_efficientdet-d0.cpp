...
extern "C" bool NvDsInferParseCustomEfficientDetD0(
 ...
 *
 * @param [x0] Bounding Box left X-Coordinate.
 * @param [y0] Bounding Box top Y-Coordinate.
 ...
 */
static NvDsInferParseObjectInfo createBBox(const float &x0, const float &y0,
 ...
 */
static void addBBoxProposal(const float x0, const float y0, const float x1, const float y1,
                            const uint &netW, const uint &netH,
                            const int maxIndex,
                            const float maxProb,
                            std::vector<NvDsInferParseObjectInfo> &bboxInfo)
{
    NvDsInferParseObjectInfo bbox = createBBox(x0, y0, x1, y1, netW, netH);
...
static std::vector<NvDsInferParseObjectInfo> decodeEffDetD0Tensor(
    const float *detections,
    const uint &netW,
    const uint &netH,
    const uint numBBoxes,
    const float probThresh,
    const uint numOutputClasses)
{
    std::vector<NvDsInferParseObjectInfo> bboxInfo;

    for (uint idx = 0; idx < numBBoxes; idx++)
    {
        const int offset = (4 + 1 + numOutputClasses) * idx;

        const float y0 = detections[offset + 0];
        const float x0 = detections[offset + 1];
        const float y1 = detections[offset + 2];
        const float x1 = detections[offset + 3];
        const float objectness = 1 / (1 + exp(-detections[offset + 4]));
        ...
        {
            float class_val = detections[offset + 5 + i];
        ...
        {
            addBBoxProposal(x0, y0, x1, y1, netW, netH, maxIndex, maxProb, bboxInfo);
        }
    }
    ...
    std::vector<NvDsInferParseObjectInfo> &objectList)
{
    ...
    static const uint kNUM_ANCHORS = 49104;
    static const float kNMS_THRESH = 0.2f;
    static const float kPROB_THRESH = 0.4f;
    static const uint kNUM_CLASSES_EFFDET = 90;
    ...
    if (kNUM_CLASSES_EFFDET != detectionParams.numClassesConfigured)
    {
        std::cerr << "WARNING: Num classes mismatch. Configured:"
                  << detectionParams.numClassesConfigured
                  << ", detected by network: " << kNUM_CLASSES_EFFDET << std::endl;
    }
    ...
    objectList.clear();
    objectList = nmsAllClasses(objects, kNUM_CLASSES_EFFDET, kNMS_THRESH);

    return true;
}

/* Check that the custom function has been defined correctly */
CHECK_CUSTOM_PARSE_FUNC_PROTOTYPE(NvDsInferParseCustomEfficientDetD0);
